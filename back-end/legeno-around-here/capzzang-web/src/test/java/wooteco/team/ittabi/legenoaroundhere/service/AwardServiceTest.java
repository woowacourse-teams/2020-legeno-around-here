...
import static wooteco.team.ittabi.legenoaroundhere.util.constants.AreaConstants.TEST_AREA_OTHER_ID;
...
import static wooteco.team.ittabi.legenoaroundhere.util.constants.SectorConstants.TEST_SECTOR_ANOTHER_REQUEST;
...
class AwardServiceTest extends ServiceTest {
    ...
    void setUp() {
        ...
        Mockito.when(mailAuthRepository.findByEmail(any()))
        ...
        postCreateRequest = new PostCreateRequest(TEST_POST_WRITING, TEST_EMPTY_IMAGES, SEOUL_ID,
            sectorId);
        Long postIdB = postService.createPost(postCreateRequest).getId();
        postCreateRequest = new PostCreateRequest(TEST_POST_WRITING, TEST_EMPTY_IMAGES, ID_IN_SEOUL,
            sectorId);
        Long postIdC = postService.createPost(postCreateRequest).getId();
        postCreateRequest = new PostCreateRequest(TEST_POST_WRITING, TEST_EMPTY_IMAGES,
    ...
    void createMonthlyPopularPostAwards() {
        ...
        assertThat(awardNames)
            .contains(awardNameDatePart + " 서울특별시 " + TEST_SECTOR_NAME.toUpperCase() + " 부문 1위");
        assertThat(awardNames)
            .contains(awardNameDatePart + " 서울특별시 " + TEST_SECTOR_NAME.toUpperCase() + " 부문 2위");
        assertThat(awardNames)
            .contains(awardNameDatePart + " 서울특별시 " + TEST_SECTOR_NAME.toUpperCase() + " 부문 3위");
        assertThat(awardNames)
            .contains(awardNameDatePart + " 종로구 " + TEST_SECTOR_NAME.toUpperCase() + " 부문 1위");
